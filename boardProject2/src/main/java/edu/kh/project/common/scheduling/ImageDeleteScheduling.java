...
// == 스프링이 해당 클래스를 객체를 만들어서 관릴르 해야함
// == Bean 등록

// @Contorlelr @Service 등등의 부모 어노테이션
...
public class ImageDeleteScheduling {
	...
	//@Scheduled(cron = "0,30 * * * * *") // 매분 0초, 30초마다 수행 
...
//		
		...
		if( !serverImageList.isEmpty()) {
			...
			for(File server : serverImageList) {
				...
				// List.indexOf(객체) = 객체가 List에 있으면 해당 인덱스반환, 없으면 -1 반환
				if(dbImageList.indexOf(server.getName())== -1) {
					// db 파일 목록  		서버파일 이름 == -1(없음)
